import { combineUsersWithMessages } from './service';
...
describe('Testing the Service', () => {
  ...
  it('should combine users with messages', () => {
    ...
    expect(combinedMessages[0].user).not.toBe(undefined);
    expect(combinedMessages[0].user.id).toBe(members[1].id);
    expect(combinedMessages[1].user).toBe(undefined);
  });
